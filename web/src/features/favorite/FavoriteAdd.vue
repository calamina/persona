...
const addFavoriteForm = useTemplateRef('addFavoriteForm')
...
const create = async () => {
  loading.value = true
  const formData = new FormData(addFavoriteForm?.value ?? undefined)
  ...
  const { data } = await addFavorite(parsedData)
  if (data) {
